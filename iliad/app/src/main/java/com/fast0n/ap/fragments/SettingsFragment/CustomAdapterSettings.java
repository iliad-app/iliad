        ...
        holder.toggle.setOnCheckedChangeListener((buttonView, isChecked) -> {
            PermissionListener permissionlistener = new PermissionListener() {
                ...
                public void onPermissionGranted() {
                    Toasty.success(context, "Permesso approvato", Toast.LENGTH_SHORT).show();
                }

                @Override
                public void onPermissionDenied(List<String> deniedPermissions) {
                    Toasty.error(context, "Permesso negato", Toast.LENGTH_SHORT).show();
                    holder.toggle.setChecked(false);
                }
            ...
            switch (position) {
                case 0:
                    if (holder.toggle.isChecked())
                        ...
                        holder.toggle.setChecked(true);
                        Intent i = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + BuildConfig.APPLICATION_ID)).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK |Intent.FLAG_ACTIVITY_CLEAR_TASK);
